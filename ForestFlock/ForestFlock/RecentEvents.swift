                    ...
                    Text("Recent Events").font(.largeTitle).bold().padding(.vertical, 50)
                        ForEach(viewModel.updates.reversed(), id: \.id){update in
                            if update.sound_class != nil {
                                ...
                                RecentWeatherEventCard(type: "Weather", latitude: viewModel.getDeviceLatitude(device: update.device_id), longitude: viewModel.getDeviceLongitude(device: update.device_id), picture: "sun.max", temperature: update.temperature!, humidity: update.humidity!, pressure: update.pressure!, timestamp: "Hello", collapsed: $collapsed).onTapGesture {
                                    viewModel.selectedDeviceID = update.device_id
                                    showingSheet.toggle()
                                }
                            }
