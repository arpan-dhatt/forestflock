...
async fn handle_connection(
    ...
    let (tx, rx) = unbounded();
    let update_list: Vec<ServerUpdate> = db
        .open_tree(b"updates")
        .unwrap()
        .iter()
        .values()
...
async fn main() -> Result<(), IoError> {
    ...
    let server_update_db = db.clone();
    tokio::spawn(async move {
        receive_server_updates(server_update_db, server_update_arc, rx).await;
    });
    ...
    while let Ok((stream, addr)) = listener.accept().await {
        tokio::spawn(handle_connection(db.clone(), state.clone(), stream, addr));
    }

    Ok(())
}

async fn receive_server_updates(
    db: Database,
    peer_map: PeerMap,
    mut event_stream: UnboundedReceiver<ServerUpdate>,
) {
    while let Some(update) = event_stream.next().await {
        ...
        for (addr, recp) in peers.iter() {
            ...
            recp.unbounded_send(Message::binary(json_bytes.clone()))
                .unwrap();
        }
    }
}
